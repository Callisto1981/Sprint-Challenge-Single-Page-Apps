...
- [ ] Explain benefit(s) using `client-side routing`?
      Everything is rendered there on the page and there is no loading pages so there's less data to process making the transitions to pages faster. If there is a timer for example, with client side routing, once the client switches to a different page, the time is able to continue counting. Where as server side routing, the timer would reset.
